...
const AddProduct: React.FC<AddProductProps> = ({ authToken, availableBrands, onBack, onSuccess }) => {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">شناسه محصول *</label>
                <input name="productId" value={formData.productId} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="productId" required />
              </div>
              <div>
            ...
            <div className="flex justify-end space-x-4 space-x-reverse">
              ...
              <button type="submit" disabled={loading} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2 space-x-reverse">
                {loading ? (<><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div><span>در حال ایجاد...</span></>) : (<><Save className="w-4 h-4" /><span>ایجاد محصول</span></>)}
              </button>
            </div>
          </form>
        </div>
