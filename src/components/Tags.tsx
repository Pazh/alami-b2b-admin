...
import { Edit, Trash2, Plus, Save, X, ChevronUp, ChevronDown, Tag as TagIcon } from 'lucide-react';
...
const Tags: React.FC<TagsProps> = ({ authToken }) => {
  ...
  const [addForm, setAddForm] = useState<Omit<Tag, 'id'>>({
    name: ''
  });
  ...
  const fetchTagById = async (id: string) => {
    try {
      const data = await apiService.getTagById(id, authToken);
      return data.data;
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to fetch tag details');
      return null;
    }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await apiService.updateTag(editingId, { name: editForm.name }, authToken);
      await fetchTags();
      setEditingId(null);
      setEditForm({});
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await apiService.deleteTag(id, authToken);
      await fetchTags();
    } catch (err) {
  ...
  const handleAdd = async () => {
    ...
    try {
      await apiService.createTag({ name: addForm.name }, authToken);
      await fetchTags();
    ...
    <div className="glass-effect rounded-2xl shadow-modern mobile-card border border-white/20">
      ...
      <div className="mobile-flex mobile-space mb-6">
        <div className="flex items-center space-x-3 space-x-reverse">
          ...
          <h2 className="text-xl lg:text-2xl font-bold gradient-text">برچسب‌ها</h2>
        </div>
      ...
      {showAddForm && (
        <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <h3 className="text-lg font-medium text-gray-900 mb-4">افزودن برچسب جدید</h3>
      ...
      <div className="hidden lg:block table-responsive">
        <table className="table-mobile">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {sortedTags.map((tag) => (
              <tr key={tag.id} className="table-mobile-row">
                <td className="table-mobile-cell">
                  {editingId === tag.id ? (
                    ...
                    <div className="mobile-text font-medium text-gray-900">{tag.name}</div>
                  )}
                </td>
                <td className="table-mobile-cell">
                  {editingId === tag.id ? (
      ...
      {sortedTags.length === 0 && !loading && (
        <div className="text-center py-8">
          <div className="text-gray-500 text-lg mb-2">هیچ برچسبی یافت نشد</div>
          <p className="text-gray-400 text-sm">برای شروع، برچسب جدیدی اضافه کنید</p>
        </div>
      )}
    </div>
  );
