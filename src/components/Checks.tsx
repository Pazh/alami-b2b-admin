...
const Checks: React.FC<ChecksProps> = ({ authToken, userId, userRole }) => {
  ...
  const [updatingSayyadiId, setUpdatingSayyadiId] = useState<string | null>(
    null
  );
  const [showMobileFilter, setShowMobileFilter] = useState(false);
  const [addForm, setAddForm] = useState<
    Omit<Cheque, 'id' | 'customerData' | 'managerData'>
  >({
    number: '',
    date: '',
    price: 0,
  ...
  const [selectedFilterCustomer, setSelectedFilterCustomer] =
    useState<any>(null);
  const [filterCustomerTimeout, setFilterCustomerTimeout] = useState<
    number | null
  >(null);

  const [selectedChequeForLogs, setSelectedChequeForLogs] = useState<
    string | null
  >(null);
  const [scrollPosition, setScrollPosition] = useState(0);

  const baseUrl =
    import.meta.env.VITE_API_BASE_URL || 'https://alami-b2b-api.liara.run/api';
      ...
      if (
        ...
        if (hasFilters) {
          ...
          if (filters.number.trim())
            filterData.number = toEnglishDigits(filters.number.trim());
          if (filters.startDate.trim())
            filterData.startDate = parseDate(
              toEnglishDigits(filters.startDate.trim())
            );
          if (filters.endDate.trim())
            filterData.endDate = parseDate(
              toEnglishDigits(filters.endDate.trim())
            );
          if (filters.price.trim())
            filterData.price =
              parseInt(toEnglishDigits(filters.price.trim())) || undefined;
          if (filters.status.trim()) filterData.status = filters.status.trim();
          if (filters.bankName.trim())
            filterData.bankName = filters.bankName.trim();
          if (filters.customerId.trim())
            filterData.customerUserId = filters.customerId.trim();
  ...
  const handleToggleSayyadi = async (
    checkId: string,
  ...
  const getCustomerName = (customerData: CustomerData) => {
    return `${customerData.account.firstName}${
      customerData.account.lastName ? ` ${customerData.account.lastName}` : ''
    }`;
  };

  const getCustomerNameFromCustomer = (customer: Customer) => {
    return `${customer.account.firstName}${
      customer.account.lastName ? ` ${customer.account.lastName}` : ''
    }`;
  };

  const getManagerName = (managerData: ManagerData) => {
    return `${managerData.firstName}${
      managerData.lastName ? ` ${managerData.lastName}` : ''
    }`;
  };
    ...
    try {
      ...
      if (userRole === RoleEnum.MARKETER) {
        ...
        response = await fetch(
          `${baseUrl}/customer-relation/filter?pageSize=20&pageIndex=0`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        response = await fetch(
          `${baseUrl}/customer-user/filter?pageSize=20&pageIndex=0`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
    ...
    try {
      ...
      if (
        ...
        const response = await fetch(
          `${baseUrl}/customer-user/filter?pageSize=10&pageIndex=0`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        const response = await fetch(
          `${baseUrl}/customer-relation/filter?pageSize=10&pageIndex=0`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
  ...
  const selectCustomer = (customer: any, isEdit = false) => {
    const customerName = `${customer.account.firstName}${
      customer.account.lastName ? ` ${customer.account.lastName}` : ''
    }`;
    ...
    <div className="glass-effect rounded-2xl shadow-modern-lg p-4 lg:p-8 border border-white/20 animate-fade-in !h-full">
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-6 space-y-4 lg:space-y-0">
        <div className="flex items-center space-x-3 space-x-reverse">
          ...
          <h2 className="text-xl lg:text-2xl font-bold gradient-text w-full">
            مدیریت چک‌ها
          </h2>
        </div>
        <div className="flex flex-col md:flex-row items-start md:items-center w-full gap-x-4 gap-y-4">
          {hasActiveFilters && (
            <button
          ...
          <div className="flex flex-row items-center gap-x-2  w-full justify-start">
          <div className="mobile-text text-gray-500 bg-white/80 rounded-xl backdrop-blur-sm text-sm w-full">
            تعداد کل: {toPersianDigits(totalCount)}
          </div>
          <div className="flex items-center w-full">

            <label className="mobile-text text-gray-700 inline text-sm w-full">
              تعداد در صفحه:
            </label>
            <select
              value={pageSize}
              onChange={(e) => handlePageSizeChange(parseInt(e.target.value))}
              className="p-2 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white/80 backdrop-blur-sm shadow-md"
            >
          ...
          <div className="flex items-center gap-x-4 justify-center w-full rounded-xl">
      ...
      {error && (
        <div className="mb-6 p-4 bg-gradient-to-r from-red-500/10 to-red-600/10 border border-red-200 !rounded-2xl shadow-lg animate-slide-up">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="noor">بانک نور</option>
                    <option value="mehr_eghtesad">بانک مهر اقتصاد</option>
                    ...
                    <div className="mt-2 max-h-40 overflow-y-auto border border-gray-200 rounded-xl bg-white shadow-lg">
                      {filterCustomerResults.map((customer) => (
                        <button
                          ...
                          className="w-full px-3 py-2 text-right text-sm hover:bg-gray-50 border-b border-gray-100 last:border-b-0"
                        >
                          {customer.account.firstName}{' '}
                          {customer.account.lastName || ''}
                        </button>
                    ...
                    <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded-lg">
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-blue-700">
                          {selectedFilterCustomer.account.firstName}{' '}
                          {selectedFilterCustomer.account.lastName || ''}
                        </span>
                        <button
                          onClick={() => {
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setAddForm({
                    ...addForm,
                    number: toEnglishDigits(e.target.value),
                  })
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setAddForm({
                    ...addForm,
                    price: parseInt(toEnglishDigits(e.target.value)) || 0,
                  })
                }
                className="input-modern"
                placeholder="0"
                ...
                <input
                  ...
                  value={
                    selectedCustomerForAdd
                      ? `${selectedCustomerForAdd.account.firstName} ${
                          selectedCustomerForAdd.account.lastName || ''
                        }`
                      : ''
                  }
      ...
      <div className="overflow-x-auto hidden lg:block">
        <table className="table-modern">
          <thead>
            <tr className="table-header">
                ...
                {showFilters.number && (
                  <div className="mt-2 flex items-center space-x-2 space-x-reverse">
                    <input
                      ...
                      onKeyPress={(e) =>
                        e.key === 'Enter' && handleFilterSubmit('number')
                      }
                      className="w-full px-3 py-2 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white/90 backdrop-blur-sm"
                      placeholder="شماره چک..."
                      autoFocus
                ...
                {showFilters.price && (
                  <div className="mt-2 flex items-center space-x-2 space-x-reverse">
                    <input
                      ...
                      onKeyPress={(e) =>
                        e.key === 'Enter' && handleFilterSubmit('price')
                      }
              ...
              <th className="px-4 lg:px-6 py-3 lg:py-4 text-right text-xs lg:text-sm font-bold text-gray-700 hidden">
                مدیر
              </th>
              <th className="px-4 lg:px-6 py-3 lg:py-4 text-right text-xs lg:text-sm font-bold text-gray-700 hidden">
                تاریخ ایجاد
              ...
              >
                ...
                <td className="px-4 lg:px-6 py-4 whitespace-nowrap hidden">
                  <div className="text-sm text-gray-900">
                    {cheque.createdDate
                      ? toPersianDigits(
                          formatISODateToPersian(cheque.createdDate)
                        )
                      : '-'}
                  </div>
                </td>
        ...
        {cheques.length === 0 && !loading && (
          <div className="text-center py-8">
            ...
            {userRole === RoleEnum.MARKETER &&
            allowedCustomerIds.length === 0 ? (
              <p className="text-gray-400 text-sm">
                هیچ مشتری به شما اختصاص داده نشده است. برای مشاهده چک‌ها ابتدا
                باید مشتری به شما تخصیص یابد.
              </p>
            ) : (
              <p className="text-gray-400 text-sm">
                چک‌های سیستم در اینجا نمایش داده می‌شوند
              </p>
            )}
          </div>
        )}
        ...
        {cheques.map((cheque) => (
          ...
          >
            ...
            <div className="flex items-center justify-between pt-3 border-t border-gray-200">
              ...
              {cheque.description && (
                <span
                  className="text-gray-400 text-xs max-w-[150px] truncate hidden"
